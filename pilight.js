...
var language_hu = {
	...
	connection_lost: "Kapcsolat megszakadt, kattintson az újratöltéshez",
	connection_failed: "Kapcsolat hiba, kattintson az újratöltéshez",
...
$(document).keydown(function(e) {
	if(e.keyCode == 72) {
		if($('#helpdlg').length == 1) {
			...
			$("<div data-theme='a' id='helpdlg' class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><b>Shortcuts</b><br /><ul><li>[ctrl] + [alt] + [shift] + t<br />Switch between tabular or vertical view</li></ul></div>")
			.appendTo($('body'));
...
function createWeatherElement(sTabId, sDevId, aValues) {
	...
	if($('#'+sDevId+'_weather').length == 0) {
		...
		if('show-temperature' in aValues && aValues['show-temperature']) {
			if(aValues['temperature'] < 3) {
				oTab.find('#'+sDevId+'_weather').append($('<div class="temperature_icon_freeze"></div><div class="temperature" id="'+sDevId+'_temp"></div>'));
			} else if(aValues['temperature'] < 12) {
				oTab.find('#'+sDevId+'_weather').append($('<div class="temperature_icon_low"></div><div class="temperature" id="'+sDevId+'_temp"></div>'));
			} else if(aValues['temperature'] < 20) {
				oTab.find('#'+sDevId+'_weather').append($('<div class="temperature_icon_med"></div><div class="temperature" id="'+sDevId+'_temp"></div>'));
			} else if(aValues['temperature'] < 27) {
				oTab.find('#'+sDevId+'_weather').append($('<div class="temperature_icon_high"></div><div class="temperature" id="'+sDevId+'_temp"></div>'));
			} else {
				oTab.find('#'+sDevId+'_weather').append($('<div class="temperature_icon_veryh"></div><div class="temperature" id="'+sDevId+'_temp"></div>'));
			}
		}
...
function updateProcStatus(aValues) {
	if('cpu' in aValues && 'ram' in aValues) {
		var obj = $('#cpu').text("\ue910 "+aValues['cpu'].toFixed(2)+"%");
		var obj = $('#ram').text("\ue90f "+aValues['ram'].toFixed(2)+"%");
